...
- Descriptions, emails, and pre-click copy must leave an additional open curiosity gap and support dual-audience design (early CTA for skimmers + continued tension for engaged readers). Vary structure deliberately so pieces do not feel formulaic. The gap must never **leak** the payoff the title/episode promises — apply the leak test in `references/youtube-planning.md`.
