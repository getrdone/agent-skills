...
| Create or review a thumbnail, graphic, moodboard, visual direction, typography, layout, or motion language | `visual` | `references/visual-system.md`; add the canonical palette skill when a theme is created or materially revised, and add Psychology only when contextual interpretation is consequential; skim `references/dynamic-symmetry-glossary.md` then `references/dynamic-symmetry.md`; for **actual image generation**, load skill **`artwork-prompts-handoff`** (prompts file for human tools—default; no paid Canva/Leonardo APIs unless user explicitly confirms credits); add `references/youtube-planning.md` for thumbnails |
