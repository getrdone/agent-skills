...
- Every new or materially revised color theme follows repository-root `skills/color-palette-composition/SKILL.md`: field/partner/spark visual weight, allocation, placement, required Itten/Munsell decisions, short theory rationale, provenance, and verification.
- Load repository-root `COLOR-PSYCHOLOGY.md` when intended/alternate audience reading, cultural or Scripture context, or a psychology claim materially changes a color decision; it supplies contextual evidence rather than a second palette generator.
...
| Create, revise, or critique a color theme, palette, color scheme, or web-page theme | `palette` | `references/palette-integration.md` + repository-root `skills/color-palette-composition/SKILL.md`; add `COLOR-PSYCHOLOGY.md` only for consequential meaning/culture/Scripture interpretation and `DESIGN-DIRECTIVES.md` only for token/gradient/implementation handoff |
| Create or review a thumbnail, graphic, moodboard, visual direction, typography, layout, or motion language | `visual` | `references/visual-system.md`; add the canonical palette skill when a theme is created or materially revised, and add Psychology only when contextual interpretation is consequential; load **`skills/dynamic-symmetry/SKILL.md`** (skim glossary; open method if needed); for **actual image generation**, load skill **`artwork-prompts-handoff`** (prompts file for human tools—default; no paid Canva/Leonardo APIs unless user explicitly confirms credits); add `references/youtube-planning.md` for thumbnails |
...
<!-- Agent: Codex | Model: GPT-5 | Thinking: not exposed | Date: 2026-09-13 | Added the lean palette lane and canonical composition/conditional-psychology routes. -->
